...
<div class="etkinlikler_baslik">Etkinlikler</div>
<?php
		if(isset($_GET['sayfa']))
			{	
			...
			*/
			...
			$SQL_tumuyeler="select * from  tablo_etkinlikler order by EtkinlikID DESC";
			...
			if($sorgu_tumuyeler)
			{
				...
				{	$sayac++;	
					...
					{
							$resimLink='img/etkinlik.png';

						
							echo'<fieldset><form><table >';
							echo'<tr><td><h3>'.$veri['EtkinlikBaslik'].'</h3></td></tr>';
							echo'<tr><td>Etkinlik Tarihi:<u>'.$veri['EtkinlikTarih'].'</u></td></tr>';
							echo'<tr><td >'.substr($veri['EtkinlikMetni'],0,80).'..</td></tr>';							
	...
	*/
			...
			if($sorgu)
			{
				...
				*/
				...
				for($i=1;$i<=$sayfa;$i++)
				{
				 echo'<a href="etkinlikler.php?sayfa='.$i.'">'.$i.'</a>';
				}
				echo'</div>';
			}
?>
