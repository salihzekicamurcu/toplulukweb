...
<div class="icerik"><!-- InstanceBeginEditable name="DegistirilebilirAlan" --><!-- InstanceEndEditable --> 
<?php
	
	if(isset($_POST['Giris']))
	{
		if(empty($_POST['YoneticiKullaniciAdi']))
		{
			echo'Kullanıcıadı Giriniz! <a href="index.php">Geri Dön</a>';
		}
		else if(empty($_POST['YoneticiSifre']))
		{
			echo'Şifrenizi Giriniz! <a href="index.php">Geri Dön</a>';
		}
		else if(empty($_POST['Guvenlik']))
		{
			echo'Güvenlik Kodunu Boş Olamaz! <a href="index.php">Geri Dön</a>';
		}
		else
		{
			/*
			...
			*/
			$YoneticiKullaniciAdi=$_POST['YoneticiKullaniciAdi'];
			$YoneticiSifre=$_POST['YoneticiSifre'];
			if($_POST['Guvenlik']!=$_POST['guvenlik_kodu'])
			{
				echo'Güvenlik Kodunu  Yanlış Girdiniz! <a href="index.php">Geri Dön</a>';
			}
			else if(strlen($YoneticiSifre)!=6)
			{
				echo'Şifreniz 6 Karakter Olmalı';
			}
			else
			{
				/*
				...
				*/
				...
				$SQL="select * from tablo_uye where KullaniciAdi='$YoneticiKullaniciAdi'";
				$sorgu=mysql_query($SQL,$baglanti);
				if($sorgu)
				{
					if($db_gelen=mysql_fetch_array($sorgu))
					{
						$UyeID=$db_gelen['UyeID'];
						if($db_gelen['Sifre']!=$YoneticiSifre)
						{
							echo'Yanlış KullanıcıAdı veya Şifre !<a href="index.php">Geri Dön</a><br>';	
						}
						else
						{
							$SQL2="select * from tablo_yonetici where UyeID='$UyeID'";
							$sorgu2=mysql_query($SQL2,$baglanti);
							if($sorgu2)
							{
								if($db_gelen2=mysql_fetch_array($sorgu2))
								{
									/*
										oturum aç
									*/
									
									$_SESSION['YoneticiAd']=$YoneticiKullaniciAdi;
									$_SESSION['YoneticiID']=$db_gelen2['UyeID'];
									...
									*/
									...
									echo'<a href="yonetim.php"><br>Devam Etmek İçin Tıklayın..</a>';
								}
								else
								{
								echo'Yanlış KullanıcıAdı veya Şifre !<a href="index.php">Geri Dön</a><br>';	
