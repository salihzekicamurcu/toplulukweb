...
<div class="icerik"><!-- InstanceBeginEditable name="DegistirilebilirAlan" --><?php

if(isset($_GET['menu']))
{
	if($_GET['menu']=='icerikEkle')
	{
		echo'<br><br>';
					echo'<form action="icerik.php"  enctype="multipart/form-data" method="post">
						<table border ="3">
						';
						echo'<tr><td width="100px">'.album_belirle().'</td></tr>';
...
else if(isset($_POST['ResimKaydet']))
{
	...
	$SQL="select DosyaYolu from tablo_galeri where GaleriID='$GaleriID' ";
    ...
    for($i=0;$i<$dosya_sayi;$i++)
	{
		...
		$_FILES['resimler']['name'][$i]="Resim".date("ymd_h-i-s").$i.'.'.$uzanti[1];
		$IcerikAdi=$_FILES['resimler']['name'][$i];
	   ...
	   if(!empty($_FILES['resimler']['name'][$i])){ 
		...
		values('$IcerikAdi','$IcerikTarih','$GaleriID','$yol','$UyeID')",$baglanti);
		if($resim_sorgu>0)
		{
			echo'Yuklendi';
		}
...
function album_belirle()
{
	include("db_baglan.php");
	$SQL="select * from tablo_galeri where AlbumAdi NOT IN('Dokuman','Video')";
	$sorgu=mysql_query($SQL,$baglanti);
	if($sorgu>0)
	{
		echo'<select name="GaleriID">';
		while($veri=mysql_fetch_array($sorgu))
		{
			echo'<option value="'.$veri['GaleriID'].'">'.$veri['AlbumAdi'].'</option>';		
		}
		echo'</select>';
