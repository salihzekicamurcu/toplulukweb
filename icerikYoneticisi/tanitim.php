	...
	if(isset($_GET['menu']))
	{
		if($_GET['menu']=='bilgiler')
		{
			...
			if($dosya)
			{
				...
				if(empty($veri))
				{
					$veri="Henuz Bilgi Girilmemiş";
				}
				echo'<form action="bilgi_guncelle.php" method="post">
				<table><tr><td><textarea  name="GenelBilgiler" rows="40px" cols="100px">'.$veri.'</textarea></td></tr>
				<tr><td><input type="submit" name="GenelBilgiGuncelle" value="Değişiklikleri Kaydet"/></td></tr></table></form>';
			}
			else
			{
				include("hata.php");
		...
		{	
			...
			if($dosya)
			{
				...
				echo'<form action="bilgi_guncelle.php" method="post">
				<table><tr><td><textarea  name="TuzukBilgisi" rows="40px" cols="100px">'.$veri.'</textarea></td></tr>
				<tr><td><input type="submit" name="TuzukBilgiGuncelle" value="Değişiklikleri Kaydet"/></td></tr></table></form>';
			}
			else
			{
				include("hata.php");
		...
		else if($_GET['menu']=='tanitim')
		{
			$dosya=fopen("../toplulukBilgileri/TanitimYazisi.txt","r");
			if($dosya)
			{
				echo'<h3>Tanıtım Yazısı Bilgisi</h3>';
				$veri="";
				while(!feof($dosya))
				{
					$veri.=fgets($dosya);
				...
				echo'<form action="bilgi_guncelle.php" method="post">
				<table><tr><td><textarea  name="TanitimBilgisi" rows="40px" cols="100px">'.$veri.'</textarea></td></tr>
				<tr><td><input type="submit" name="TanitimBilgiGuncelle" value="Değişiklikleri Kaydet"/></td></tr></table></form>';
			}
			else
			{
				include("hata.php");
		...
		else if($_GET['menu']=='YonetimKurulu')
		{
			
			@$dosya=fopen("../toplulukBilgileri/YonetimListesi.txt","r");
			if($dosya)
			{
				echo'<h3>Yönetim Kurulu Üyeleri</h3>';
				$veri="";
				while(!feof($dosya))
				{
					$veri.=fgets($dosya);
				...
				echo'<form action="bilgi_guncelle.php" method="post">
				<table><tr><td><textarea  name="YonetimKuruluUyeleri" rows="40px" cols="100px">'.$veri.'</textarea></td></tr>
				<tr><td><input type="submit" name="YonetimKuruluGuncelle" value="Değişiklikleri Kaydet"/></td></tr></table></form>';
			}
			else
			{
				include("hata.php");
