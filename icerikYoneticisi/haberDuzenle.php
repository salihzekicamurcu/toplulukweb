...
<?php
	...
	{
		if(!empty($_GET['HaberID']))
		{
			echo'Duzenle'.$_GET['HaberID'];
			$guncelle_id=$_GET['HaberID'];
			include("db_baglan.php");
			$SQL="select * from tablo_haberler where HaberID='$guncelle_id'";
			...
			if($sorgu_getir>0)
			{
				...
				{
					echo'<br><br>';
					echo'<form action="haberDuzenle.php" method="post">
						<table border ="3">
						';
						echo'<tr><th>Haber Başlığı</th><td width="100px"><input  type="text" size="100px" value="'.$gelen_veri['HaberBaslik'].'" name="HaberBaslik"/></td></tr>';
						echo'<tr><th>Haber Tarih</th><td><input type="hidden" value="'.date("Y-m-d H:i:s").'" name="HaberTarih"/>'.date("Y-m-d H:i:s").'</td></tr>';
						echo'<tr><th>Haber Metni</th><td><textarea name="HaberMetni" rows="20px" cols="100px">'.$gelen_veri['HaberMetni'].' </textarea></td></tr>';
						...
						echo'<input type="hidden" name="guncelle_HaberID" value="'.$guncelle_id.'"/>';
	...
	else if($_POST['Duzenle'])
	{
			...
			*/
			...
			else
			{
				...
				$UyeID=$_POST['UyeID'];
				$guncelle=$_POST['guncelle_HaberID'];
				/*$HaberBaslik;
							$HaberMetni;
							$HaberTarih;
							$UyeID;
							...
							$SQL_haber="update tablo_haberler set HaberBaslik='$HaberBaslik',HaberMetni='$HaberMetni',HaberTarih='$HaberTarih'
							where HaberID='$guncelle'";			
						$sorgu_haberekle=mysql_query($SQL_haber,$baglanti);
						if($sorgu_haberekle>0)
						{
							echo'<br>Haber Düzenlendi';
						}
						else
						{
							echo'Düzenlenemedi';
